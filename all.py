...
Original file is located at
    https://colab.research.google.com/drive/1T0_s3lpb_Bb0ZJiR768ed3JMDHZNkT0-
"""
...
class Data():
    def __init__( self ):
        ...
        self.the_list =    ['crash',
                            'spike','forecast_vol', 'USD Index-T-1', 
    ...
    def execute_all( self, set_date_inputs, st_ret_inputs,
                     ...
                     ma_inputs, create_labels_inputs, regime = True,
                     low = True, med = True, high = True, kurt = True ):
        ...
        if regime:
            self.add_regime_probs(low = low, med = med, high = high)
        if kurt:
            self.add_kurt()
        self.create_labels( **create_labels_inputs )

    def set_date_range( self, start_date = "2000-01-01", end_date = "2020-06-19" ):
    ...
    def add_trends( self ):
        df = dailydata.get_daily_data('recession', 2004, 1, 2019, 10, geo = 'BR')
        self.train_data = pd.merge( left = self.train_data, right = df, how = 'inner', on = 'Date', suffixes = ( False, False ) )
        self.the_list.append( 'kurt' )
...
rosemary_spike = Classifierz( data, ttype = 'spike' )
rosemary_spike.prepare_data( 20, use_val_data = True, cs = 'spike' )
ext = ExtraTreesClassifier( **ext_spike_hp )
rosemary_spike.add_classifier( ext, 'ext_spike' )
rosemary_spike.train()
rosemary_spike.make_pred()

"""kurt included"""

f1_score( rosemary_spike.results['Target'], rosemary_spike.results['ext_spike'] )
...
rosemary_spike = Classifierz( data, ttype = 'spike' )
#rosemary_crash = Classifierz( data_, ttype = 'crash' )

rosemary_spike.prepare_data( 20, use_val_data = True, cs = 'spike' )
#rosemary_crash.prepare_data( 20, use_val_data = True, cs = 'crash' )


knn_spike_hp = {'leaf_size': 189, 'n_neighbors': 3, 'p': 1, 'weights': 'distance'}
knn_crash_hp = {'leaf_size': 190, 'n_neighbors': 3, 'p': 1, 'weights': 'distance'}
...
ext_crash_hp = {'criterion': 'entropy',
            'max_depth': 212,
            'max_features': 0.8775,
            'min_samples_leaf': 2,
            'min_samples_split': 3,
...
lgbm_spike_hp = {'boosting_type': 'dart', 'n_estimators': 197, 'num_leaves': 49}
lgbm_crash_hp = {'boosting_type': 'dart', 'n_estimators': 345, 'num_leaves': 85}

rf_spike_hp = {'criterion': 'entropy',
            'max_depth': 162,
            'max_features': 0.91,
            'min_samples_leaf': 4,
            'min_samples_split': 2,
...
bag_crash_hp = { 'bootstrap' : False, 'bootstrap_features': True, 'n_estimators': 45 }
bag_spike_hp = { 'bootstrap' : False, 'bootstrap_features': True, 'n_estimators': 79 }


knn_spike = KNeighborsClassifier( **knn_spike_hp )
knn_crash = KNeighborsClassifier( **knn_crash_hp )

ext_spike = ExtraTreesClassifier( **ext_spike_hp )
ext_crash = ExtraTreesClassifier( **ext_crash_hp )


lgbm_spike = LGBMClassifier( **lgbm_spike_hp )
lgbm_crash = LGBMClassifier( **lgbm_crash_hp )

bag_spike = BaggingClassifier( **bag_spike_hp )
bag_crash = BaggingClassifier( **bag_crash_hp )

rf_spike = RandomForestClassifier( **rf_spike_hp )
rf_crash = RandomForestClassifier( **rf_crash_hp )

cat_spike = CatBoostClassifier( random_state = 23 )
cat_crash = CatBoostClassifier( random_state = 23 )

#stack = StackingCVClassifier( classifiers = [ ext ], shuffle = False, use_probas = True, cv = 5, meta_classifier = CatBoostClassifier( random_state = 32 )) 

rosemary_spike.add_classifier( knn_spike, 'Knn_spike' )
rosemary_spike.add_classifier( ext_spike, 'Ext_spike' )
rosemary_spike.add_classifier( cat_spike, 'CatB_spike' )
rosemary_spike.add_classifier( lgbm_spike, 'LGBM_spike' )
rosemary_spike.add_classifier( rf_spike, 'RF_spike' )
rosemary_spike.add_classifier( bag_spike, 'BAG_spike' )
...
"""
rosemary_crash.add_classifier( knn_crash, 'Knn_crash' )
rosemary_crash.add_classifier( ext_crash, 'Ext_crash' )
rosemary_crash.add_classifier( cat_crash, 'CatB_crash' )
rosemary_crash.add_classifier( lgbm_crash, 'LGBM_crash' )
rosemary_crash.add_classifier( rf_crash, 'RF_crash' )
rosemary_crash.add_classifier( bag_crash, 'BAG_crash' )


rosemary_crash.train()
"""
...
rosemary_spike.make_pred()
#rosemary_crash.make_pred()

list(rosemary_spike.data.keys())

"""**NO Regimes Spike**"""

f1_score( rosemary_spike.results['Target'], rosemary_spike.results['Ext_spike'] )

f1_score( rosemary_spike.results['Target'], rosemary_spike.results['Knn_spike'] )

f1_score( rosemary_spike.results['Target'], rosemary_spike.results['LGBM_spike'] )

f1_score( rosemary_spike.results['Target'], rosemary_spike.results['RF_spike'] )

f1_score( rosemary_spike.results['Target'], rosemary_spike.results['BAG_spike'] )

f1_score( rosemary_spike.results['Target'], rosemary_spike.results['CatB_spike'] )
...
rosemary_spike.results

rosemary_spike.data.spike[ rosemary_spike.results.index ]

rosemary_spike.results

rosemary_crash.results